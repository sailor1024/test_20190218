...
THREE.OrbitControls = function ( object, domElement ) {
	...
	function handleMouseMoveRotate( event ) {
		if(modeSwitch.isOrthographic()){//zidong add 20180914
			...
			rotateEnd.set(event.clientX, event.clientY);
			orthoCamera.rotateZ(rotateDelta.x * 0.001);
			return;
		}
	...
	function handleMouseMovePan( event ) {
		if(modeSwitch.isOrthographic()){//zidong add 20180914
			...
			panEnd.set(event.clientX, event.clientY);
			var localTransVector3 = new THREE.Vector3(panDelta.x*0.08,panDelta.y*-0.08,0).applyQuaternion(orthoCamera.quaternion);
	...
	function handleTouchMoveRotate( event ) {
		...
		if(modeSwitch.isOrthographic()){//zidong add 20180914
			var tempEvent = new THREE.Vector2(event.touches[0].pageX, event.touches[0].pageY); 
			rotateDelta.subVectors(tempEvent,rotateEnd).multiplyScalar(scope.rotateSpeed);
			rotateEnd.set(event.touches[0].pageX, event.touches[0].pageY);
	...
	function handleTouchMoveDollyPan( event ) {
		...
		if ( scope.enablePan ) {
			if(modeSwitch.isOrthographic()){//zidong add 20180914
				var eventTouchPan = new THREE.Vector2(event.touches[0].pageX, event.touches[0].pageY);
				panDelta.subVectors(panEnd, eventTouchPan).multiplyScalar(scope.panSpeed);
				panEnd.set(eventTouchPan.x, eventTouchPan.y);
				var localTransVector3 = new THREE.Vector3(panDelta.x*0.5,panDelta.y*-0.5,0).applyQuaternion(orthoCamera.quaternion);
				orthoCamera.position.add(localTransVector3);
			}else{
				var x = 0.5 * ( event.touches[ 0 ].pageX + event.touches[ 1 ].pageX );
				var y = 0.5 * ( event.touches[ 0 ].pageY + event.touches[ 1 ].pageY );
...
function TextureManager(renderer, cubeUrls, smallUrls, cbLoadComplete, cacheSize){
    ...
    this.cacheSize = cacheSize || 30; //缓存大小
...
function initPathAndExt(){
    ...
    if(isSupportWebp)
    {
        ...
        // urlInit = rootPath+'lwebp/1.webp';
    }
    // else{
    //     urlInit = rootPath+'/ljpeg/1.jpg';
    // }
}
...
//模型为空时只显示logo,屏蔽其他显示
$.ajax({
    url:rootPath + "model.json?ver="+Math.random(),
...
function onTouchStart ( event ) {
    ...
    var size = renderer.getSize();
    var mousex = (touchX / size.width) * 2 - 1;
    var mousey = -(touchY / size.height) * 2 + 1;
...
(function freeTimeToLoadTexture(){
    freeTimer= setInterval(function(){
        if(new Date().getTime()-timeRecord>=5000){
