$(function() {
  ...
  $('.addMattertag').click(function(ec) {
    ...
    // var description = $(".description").val();
    var description = $('.title-box>textarea').val();
    ...
    } else if (uploadSoure == '' && $('.content-box>textarea').val() == '') {
    ...
    } else if ($('.content-box>textarea').val() != '') {

      // 保存标题信息与内容信息开始
        let titleShow = $('.title-box>textarea').val();
        let titleContent = $('.content-box>textarea').val();

        // 设置标签内容显示开始
        $('#'+OnMatterTag + '>p[class=Child1-title]').text(titleShow);
        $('#'+OnMatterTag + '>p[class=Child2-content]').text(titleContent);
        // 设置标签内容显示结束

        // 保存到modeldata中
        let arr = modelData.tags
        for (let i = 0; i < arr.length; i++) {
          if (arr[i].titleId === OnMatterTag) {
            arr[i]['title-show'] = titleShow;
            arr[i]['title-content'] = titleContent;
            sendAjax();
            console.log(arr);
          }
        }
    ...
    // let arr = modelData.tags
    // for (let i = 0; i < arr.length; i++) {
    //   if (arr[i].url === 'kangyun3d') {
    //     arr[i].url = uploadSoure;
    //     arr[i].description = description;
    //     $('#'+ arr[i].sid + '').attr('datasource', uploadSoure)
    //     sendAjax();
    //     console.log(arr)
    //   }
    // }
...
$('.title-box>textarea').bind('input propertychange', function() {
  // 检测当前字符数量
     ...
     $('#'+ OnMatterTag +'>p[class=Child1-title]').text($(this).val());
     let arr = modelData.tags;
     for (let i = 0; i < arr.length; i++) {
        if (arr[i].titleId === OnMatterTag) {
          arr[i]['title-show'] = $(this).val();
          sendAjax();
        }
     }
});
$('.content-box>textarea').bind('input propertychange', function() {
  // 检测当前字符数量
     ...
     $('#'+ OnMatterTag +'>p[class=Child2-content]').text($(this).val());
     let arr = modelData.tags;
     for (let i = 0; i < arr.length; i++) {
        if (arr[i].titleId === OnMatterTag) {
          arr[i]['title-content'] = $(this).val();
          sendAjax();
        }
     }
});
