...
var rootPath = '4aa887aedd5396161b4f8250712f3c85/';
var get_modelPath = ''

if (getQueryString('p') != null) {
    // rootPath = "path/" + getQueryString('p') + '/';
    rootPath = 'path/' + getQueryString('p') + '/';
    get_modelPath = getQueryString('p');
}
...
let pimagePath = rootPath + 'preview.png';
var limagePath = rootPath + 'ljpeg/';
var fileExt = ".jpg"; //webp
...
var get_model = function() {
    ...
    return new Promise((resolve, reject) => {
        $.post(phpedit_url + "edit/get_model", {
            path: get_modelPath
        }, function(data) {
...
function mainOnClick(e) {
    ...
    // return 
    if (switchAdd) {
        anchorManager && anchorManager.creatVideo(mousex, mousey);
        switchAdd = !switchAdd;
        if (FileType == 'Uploading') {
            $('.MapVideoContent-choose').css('display', 'flex');
        } else {
            $('.MapVideoContent-choose').hide();
        }
    ...
    if (modeSwitch.isWalkingMode()) {
        ...
        if (minMouseDist < DIST_THRESHHOLD) {
            if (minMouseIndex != -1) {
                textureManager.loadTexture(minMouseIndex, TagControlWalk)
...
function animate() {
    ...
    // console.log("pointerLockControl.rotation",pointerLockControl.getObject().rotation);
    //guohua add
