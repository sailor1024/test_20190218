...
THREE.PointerLockControls = function(camera) {
    ...
    var onTouchMove = function(event) {
        // event.stopPropagation();
        // if(pointerLockControl.prohibitionMoveAndClick === true) return ;
    ...
    this.dispose = function() {
        ...
        document.removeEventListener('touchstart', onTouchStart, false);
        document.removeEventListener('touchmove', onTouchMove, false);
        document.removeEventListener('touchmove', onMouseWheel, false);
    };
...
/**
 * PointerLockControls end/ 20181122
 */
...
function AnchorManager(points, size, cameraHeight, scene, pointPath) {
    ...
    // this.matLoading = new THREE.MeshBasicMaterial({
    //     //alphaTest: 0.5,
    //     color: 0x000000,
    //     side:THREE.DoubleSide,
    //     depthTest:true,
    //     transparent:true,
    //     opacity:0.5,
    //     map: textureLoader.load("image/loading.gif")
    //     //map: textureLoader.load("image/point5.png")
    // });
    ...
    for (var i = 0; i < points.length; ++i) {
        ...
        var quad = new THREE.Mesh(geometry, matNormal);
    ...
    var geometry2 = new THREE.PlaneBufferGeometry(size * 0.8, size * 0.8, 1);
    this.posHint = new THREE.Mesh(geometry2, this.matHint);
    this.posHint.renderOrder = 2;
    //    this.posHint.position.set(1, 1,1);
...
AnchorManager.prototype.hideHint = function(point) {
    // console.log(point)
    // this.anchorPoint.copy(point);
    // this.anchorNormal.copy(normalScope);
    // this.posHint.position.set(0);
    // this.posHint.lookAt(point.add(normalScope));
}
// AnchorManager.prototype.updateThetaLength
const PI = 3.1415926;
AnchorManager.prototype.loadingTexture = function(targetIndex, loaded) {
    ...
    // setTimeout(function(){
    //     self.quadArray[targetIndex].material.map = self.highlightTex1; 
    // },2000)
    // var geometry = new THREE.RingGeometry(0.11, 0.14,30, 30,0,thetaLength );
    // geometry.rotateX(-Math.PI/2);
    // var material = new THREE.MeshBasicMaterial( { color: 0xffffff, side: THREE.DoubleSide, depthTest:false } );
    // var mesh = new THREE.Mesh( geometry, material );
    // mesh.position.set(viewPoint[targetIndex].x,viewPoint[targetIndex].y - 1.7+0.3 , viewPoint[targetIndex].z);
    //     scene.add( mesh );
}
AnchorManager.prototype.setVisible = function(bool) {
    for (var i = 0; i < this.quadArray.length; ++i) {
        this.quadArray[i].material.visible = bool;
    };
};
...
function MaterialSwitch(group) {
    ...
    this.matCube = new THREE.ShaderMaterial({
        ...
        //side: THREE.BackSide
        vertexShader: `vec4 quaternionMul(vec4 q, vec4 r) { vec3 qv = q.xyz; vec3 rv = r.xyz; return vec4( cross(qv, rv) + qv * r.w + q.w * rv, q.w * r.w - dot(qv, rv)); } vec4 rotate(vec4 q, vec4 p) { vec4 c = vec4(-1.0 * q.xyz, q.w); vec4 t = quaternionMul(q, p); return quaternionMul(t,
            c); } uniform vec3 viewPoint1; uniform vec3 viewPoint2; uniform vec4 q1; uniform vec4 q2; varying vec3 vUv1; varying vec3 vUv2; void main() { vec4 worldPosition = modelMatrix * vec4(position, 1.0); vUv1 = worldPosition.xyz - viewPoint1; vec4 r
            = rotate(q1, vec4(vUv1, 1.0)); vUv1 = r.xyz/r.w; vUv1.x = - vUv1.x; vUv2 = worldPosition.xyz - viewPoint2; r = rotate(q2, vec4(vUv2, 1.0)); vUv2 = r.xyz/r.w; vUv2.x = - vUv2.x; vec4 mvPosition = modelViewMatrix * vec4( position, 1.0 ); gl_Position
            = projectionMatrix * mvPosition; }`,
        fragmentShader: `uniform samplerCube texture1; uniform samplerCube texture2; uniform float progress; varying vec3 vUv1; varying vec3 vUv2; void main() { vec4 color1 = textureCube(texture1, vUv1); vec4 color2 = textureCube(texture2, vUv2); gl_FragColor = mix(color1, color2,
        progress); }`,
    });
...
Object.assign(CameraController.prototype, {
    isEnableRotate() {
        ...
        // return !this.moving || this.onTexutueLoaded;
    },
    setTimeTotal(time) {
...
function TextureManager(renderer, cubeUrls, smallUrls, cbLoadComplete, cacheSize) {
    ...
    this.cacheSize = cacheSize || 8; //缓存大小
    ...
    this.frameCount = 1;
    this.netSpeed = 0; //检测网速,越小越快
}
Object.assign(TextureManager.prototype, {
    // ifcanmove=false 只加载纹理,不移动
    loadTexture(index, ifcanmove) {
        ...
        if (r >= 0) { //已加载
            ...
            pointerLockControl.prohibitionMoveAndClick = false;
...
Object.assign(AnimateControl.prototype, {
    ...
    getState() {
        ...
        if (this.execFrames.length == 0 && this.currentFrame >= this.currentFrames.length) return AnimateControl.Stoped;
    ...
    update() {
        ...
        //开始播放
        while (this.currentFrame < this.currentFrames.length && this.currentFrames[this.currentFrame].start <= this.timeElapsed) {
...
Object.assign(Module3D.prototype, {
    ...
    calcPosition(point, normal, distance) {
        return new THREE.Vector3(point.x + normal.x * distance, point.y + normal.y * distance, point.z + normal.z * distance)
    },
    //当前点索引
    getBubbleIndex() {
        return targetIndex;
...
! function() {
    ...
    Multithread.prototype._prepare = function(fn, type) {
        ...
        var script = this._worker[type].toString().replace(/^.*?[\n\r]+/gi, '').replace(/\}[\s]*$/, '').replace(/\/\*\*\/name\/\*\*\//gi, name).replace(/\/\*\*\/func\/\*\*\//gi, fnStr);
...
$.ajax({
    ...
    error: function(er) {
        ...
        $('body').html('<img src="image/logo.png" style="position:absolute;width:30vw;top:50%;left:50%;transform:translate(-50%,-50%)" />');
    }
});
...
$.getJSON(rootPath + "model.json?ver=" + Math.random(), function(obj, err) {
    modelData = obj;
    var cubeUrls = [];
    var smallUrls = [];
...
function init() {
    ...
    if (document.URL.indexOf('nomodel') == -1) {
        if (modelData.modelSmall && modelData.modelSmall.length > 0) {
            ...
            loader.load(rootPath + modelData.modelPath, function(gltf) {
                textureManager.netSpeed = (Date.parse(new Date()) - startTime) / 1000; //网速检测
                initLoadedModel(gltf.scene);
                onWindowResize();
            }, undefined, function(error) {
                initLoadedModel(null);
...
function onMouseMove(e) {
    if (e.clientX != posMouseDown.x || e.clientY != posMouseDown.y) { //有时mousedown后会直接触发mousemove
        ismouseMoved = true;
    }
    ...
    if (debugMode && cameraController) {
        // var mousex = (e.clientX / window.innerWidth) * 2 - 1;
        // var mousey = -(e.clientY / window.innerHeight) * 2 + 1;
        //console.log(getMousePos3D(e.clientX, e.clientY));
        ...
        if (modeSwitch.isOrthographic()) {
            ...
            vector.normalize();
            if (vector.y < 0) {
                vector.multiplyScalar(-1.0 / vector.y * pos.y);
...
function onMouseUp(e) {
    ...
    pointerLockControl.touchX = e.clientX;
    pointerLockControl.touchY = e.clientY;
    if (Math.abs(pointerLockControl.touchX - posMouseDown.x) < 300) {
        return;
    }
    pointerLockControl.keepMove = true;
    // mainOnClick(e);
}
        ...
        if (matched != -1) {
            if (1 - maxDot < 0.005) {
                ...
                for (var i = 0; i < nearPoints.length; i++) {
                    ...
                    const PI = 3.1415926;
                    if (curAngle < PI / 2.0 && curAngle >= -PI / 2.0) {
                        if (curDist < minDist) {
                            minDist = curDist;
...
function onModeChange() {
    ...
    orbitControl.enabled = modeSwitch.isOverallMode() || modeSwitch.isOrthographic(); //zidong add 20180914
    pointerLockControl.enabled = false;
}
...
function setDefaultOverallView() {
    ...
    anchorManager.setVisible(false);
...
function onPreLoadTexture(targetIndex) {
    ...
    var j = targetIndex;
    var funcInADifferentThread = MT.process(function() {}, function() {
        for (var i = 0; i < nearPoints.length; i++) {
...
function onTexutueLoaded(index, texture) { //纹理加载完毕
    if (targetIndex == index) {
        ...
        cameraController.onTexutueLoaded = true;
        // minPointIndex
        // switchToWalkingMode();
    }
}
...
function switchToWalkingMode() { //从总览模式切换到漫游模式
    // if(materialSwitch.loaded() && !modeSwitch.isWalkingMode()){
