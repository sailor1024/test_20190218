...
// var camera = copy(cameraController.getCurrentCamera());
// Create a reticle
...
camera.add(reticle);
// scene.add(camera);
// camera.position.set(-3.086956,0,2.809767);
...
navigator.getVRDisplays().then(function(vrDisplays) {
	...
	if (vrDisplays.length) {
		...
		// console.log("controlsVR",controlsVR);
	...
	// displays, so provide controls for a monoscopic desktop view
		...
		// console.log("cameraController.getCurrentCamera()", cameraController.getCurrentCamera());
		controlsVR = new THREE.OrbitControls(camera);
		// controlsVR.target.set(0, 0, -1);
...
function animateVR(timestamp) {
	...
	// camera.position.x = cameraController.getCurrentCamera().position.x;

	if (cameraController != undefined) {
		// console.log(pointerLockControl.getObject().position);
		// console.log(camera.position);
		// camera.position.set(-3.086956,0,2.809767);
	}
