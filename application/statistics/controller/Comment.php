...
class Comment { 
    ...
    * @history     TD 20190403 增加代码，注释原代码，同时写些粗口备注 
    */
    ...
    /*    empty($data['get_data_type']) && $data['get_data_type'] = 'items';  
        ...
        //我想讲粗口，改改改，深沉地快乐着
           ...
           if (!isset($data['get_data_type'])) {
              $data['get_data_type'] = 'items';  
           } 

            $userData['data']['type'] = $data['get_data_type'];
            $userData['data']['company_id'] = $tokenData['company_id'];
            $userData['data']['user_id'] = $tokenData['user_id'];
            $judgement = Db::table('items_praise')->where('items_dirname', $data['items_dirname'])->find();
            $userData['data']['dirname'] = ($judgement['items_dirname'])?$judgement['items_dirname'] : '';
             return $userData ;
    }
     ...
     * @history    重新写了这个接口
     */
    public function praiseWeekStatistics_two() {
        try{ 
            $data = input('param.');
            $time = isset($data['time'])?$data['time']:time(); 
            $oneWeek = $this->weekInterval($time);//一周七天的日期
           ...
           return json(['code' => 416, 'message' =>$e->getMessage() , 'data' => array()]);
        }
    }
    ...
    public function get_share_items_link() {
        ...
        try{
            ...
            $class_data = new \app\statistics\controller\Data();
            ...
            if ($today_sum != 0  && $nb_visits != 0) {
                $today_relative_rate = ($today_sum / $nb_visits);
            } 
            //设置昨天
            $yesterday_relative_rate = 0; // 昨天的分享率
            if ($yesterday_sum != 0 && $yesterday_nb_visits != 0) {
                $yesterday_relative_rate = ($yesterday_sum / $yesterday_nb_visits);
            }
            //设置相对比率
            $all_relative_rate = ($today_relative_rate - $yesterday_relative_rate) ; //增减比率，今天减去昨天
            $all_relative_rate_status = 0; //增降状态
            if ($all_relative_rate > 0) { //证明
                $all_relative_rate_status = 1;
            } else if ($all_relative_rate < 0 ) {
            ...
            $get_data = [
                'sum' => $today_sum, //今天的分享数量
                //'yesterday_sum' => $yesterday_sum, // 
                //'yesterday_nb_visits' => $yesterday_nb_visits, // 
                'nb_visits' => $nb_visits, //今天的访客数量
                'all_relative_rate' => $all_relative_rate, //增降比率
    ...
    /*
      记录发表项目的评论
        get param
         ...
         - param  user_photo     string      发表评论的头像链接
         - param  dirname        string      评论的项目

        return  message => array(
            'code'=> 200|201|202|203|204|205  //200=>成功，201=>发表失败，202=>参数不齐，203=>没有该项目,204=>项目不能为空,205=>内容不能为空
            'message'=>'返回操作的提示信息' , // 
            'data'=>array(
                'id',  //id
                'create_time', //评论时间
    ...
    */
    public function report_items_comment(){
        // 641b3ad9547fbd821d764372e150c7ac
        $dirname = input('param.dirname'); //
        $content = input('param.content');
        $user_photo = input('param.user_photo',''); //默认可以为空

        $get_insert_message  = []; //初始化返回的data

        if(!isset($dirname) ||  !isset($content) ) {
            return json_encode(['code'=>202 , 'message'=>'参数不齐' , 'data'=>[] ]);
        }else{
            if(empty($dirname)){
                return json_encode(['code'=>204 , 'message'=>'项目不能为空' , 'data'=>[] ]);
            }

            if(empty($content)){
                return json_encode(['code'=>205 , 'message'=>'内容不能为空' , 'data'=>[] ]);
            }
        }

        $get_items = Db::name('items')->where(['dirname' => $dirname ])->field('company_id,user_id,id')->find();
        if(empty($get_items)){
            return json_encode(['code'=>203 , 'message'=>'没有该项目 ' , 'data'=>[] ]);
        
        }

        try{
            
            $find_last_comment = Db::name('items_comment')->field('id,list_num')->order('list_num','desc')->find();
            $list_num = 1 ;// 默认楼层
            if(!empty($find_last_comment)){
                $list_num =  intval($find_last_comment['list_num'] ) + 1;
            }
            $now_time = time();
            $save_data = [
            ...
            $save_to_data = Db::name('items_comment')->insert($save_data );
            if($save_to_data ){ 
                //返回最后条信息
                $get_insert_message = Db::name('items_comment')
                                    ->where(['dirname'=>$dirname,'list_num' => $list_num])
                                    ->field('id,create_time,dirname,list_num,content,user_photo,praise_num')
                                    ->find();
            }

        }catch(\Exception $ee){
            return json_encode(['code'=>201,'message'=>$ee->getMessage(),'data'=>[] ]);
        }

        return json_encode(['code'=>200,'message'=>'评论成功','data'=>$get_insert_message ]);
    ...
    /*
        ...
        get param
         - param  id        int      评论的id
        code => 200 成功，201=>失败，203=>参数不齐，202=>没有给id的项目
    */
    public function praise_to_comment(){
        $id = input('param.id');
        if(empty($id)){
            return json_encode(['code'=>203 , 'message'=>'id 不能为空' , 'data'=>[] ]);
        }

        $get_id_message = DB::name('items_comment')->where('id',$id)->field('praise_num,id')->find();
        if(empty($get_id_message)){
            return json_encode(['code'=>202 , 'message'=>'该id不存在' , 'data'=>[] ]);
        }
        try{
            $praise_num = intval( $get_id_message['praise_num'])  + 1 ;
            
            $save_data = Db::name('items_comment')->where('id',$id)->update(['praise_num'=> $praise_num]);
            if(!$save_data ){
                return json_encode(['code'=>201 , 'message'=>'点赞失败' , 'data'=>[] ]);
            }
           
        }catch(\Excepton $eee){
            return json_encode(['code'=>201 , 'message'=>$eee->getMessage() , 'data'=>[] ]);
        }

        return json_encode(['code'=>200,'message'=>'点赞评论成功','data'=>[] ]);
    ...
    /*
        ...
        return  message => array(
            'code'=> 200|201|204 //200=>成功，201=>获取失败,204=>项目不能为空 
            'message'=>'返回操作的提示信息' , // 
            'data'=>array(
                'id',  //id
                'create_time', //评论时间
    ...
    */
    public function get_items_comment_list(){
        $dirname = input('param.dirname'); //
        ...
        $get_comment_list = Db::name('items_comment')
                                    ->where(['dirname'=>$dirname])
                                    ->field('id,create_time,dirname,list_num,content,user_photo,praise_num,top_time')
                                    ->order(['top_time'=>'desc','create_time'=>'asc'])
                                    ->limit(20)
                                    ->select();

        return json_encode(['code'=>200,'message'=>'获取评论成功','data'=>$get_comment_list ] );                           
