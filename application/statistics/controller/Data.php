...
class Data {
   ...
   //如果对接口的操作的返回不明确，登录到后台  index.php?module=API&action=listAllAPI&date=yesterday&period=day&idSite=9
    ...
    protected function getParam($entity) {
        ...
        if (empty($entity['items_dirname'])) {  
            ...
            if ($entity['items_dirname'] == 'all' || $entity['method'] == 'MultiSites.getAll') { //如果 items_dirname = all ，就将 idSite = all ,将获取所有的数据
                ...
                $param_items = [];
                $param_items['items_dirname'] = $entity['items_dirname'] ;//items_dirname == (company or user or dirname) 
                $get_site_data = $items->getSite($param_items); //获取到matoma的保存到的siteId数据
    ...
    //获取总访客信息,比较昨天和今天
    public function all_VisitsSummary_get() {
        //设置默认天数
        $now_time = time();
    ...
    //获取总访客信息，历史访客
    public function history_VisitsSummary_getVisits() {
        //设置默认天数
        $now_time = time();
        $today_time = date("Y-m-d",$now_time);
       // $yesterday_time = date("Y-m-d",($now_time - 60*60*24)); //昨天
    ...
    public function active_users_VisitorInterest() {
        //设置默认天数
        $now_time = time();
        $today_time = date("Y-m-d",$now_time);
        $yesterday_time = date("Y-m-d",($now_time - 60*60*24)); //昨天
          ...
          // $entity_param['get_data_type'] = 'company';
            ...
            $return_data = array(
                'data' => [
                    ...
                    'today_data_all_sum' =>$temp_today_data_all_sum, //今天所有用户总量
    ...
    //获取访客信息, 当前周的每日访客量
    public function one_day_week_VisitsSummary_getVisits() {
        $return_data = array(
            'data' => [],
            'message' => '',
            'code'=> 201,
    ...
    */
     ...
     //获取某个项目访问量
     public function get_VisitsSummary_getVisits() {
        $entity_param = input('param.');
